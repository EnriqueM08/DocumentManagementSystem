...
<html>
    ...
    <body>
        <?php
            ...
            if (isset($_REQUEST['err']) && ($_REQUEST['err'] == "invalidLoan"))
            {
                echo '<div class="alert alert-danger alert-dismissable">';
                echo '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>';
                echo 'The loan id you entered is invalid must be an integer up to 8 characters!</div>';
            }
            ...
            if(isset($_POST['submit']))
            {
                ...
                if(!preg_match('/^[0-9]{8}$/', $loanNum))
                    redirect("upload_new.php?err=invalidLoan");
                $docType=$_POST['docType'];
                $tmpName=$_FILES['userfile']['tmp_name'];
                $fileSize=$_FILES['userfile']['size'];
                $fileType=$_FILES['userfile']['type'];
                if($fileType != 'application/pdf') {
                    redirect("https://ec2-18-222-116-88.us-east-2.compute.amazonaws.com/upload_new.php?msg=notPdf");
                }
                else{
